#!/usr/bin/env python3
"""
corral-tui — terminal front-end for the Corral daemon.

A read-only-looking dashboard that is actually a full controller: it renders
~/.corral/state.json and drives the guard by dropping command files into
~/.corral/cmd/ (exactly like the menubar app). stdlib only (curses).
...
DAEMON = os.path.join(os.path.dirname(os.path.abspath(__file__)), "corrald.py")
...
def agent_row(sess, reason, since):
    """One HERD row: <project> — <activity or status phrase> · <family> · <ago>."""
...
def draw(win, s):
    ...
    safe_addstr(win, 0, 2, "CORRAL", cp(C_TITLE) | curses.A_BOLD)
    ...
    # ---- THE HERD — Claude agents on this Mac. Omitted entirely when the
    ...
    if isinstance(ag, dict):
        ...
        for c in (ag.get("collisions") or []):
            ...
            break
        herd = ag.get("herd") or {}
        counts = (f"{herd.get('working', 0)} at work · "
                  f"{herd.get('done_today', 0)} done today · "
                  f"{herd.get('needs_you', 0)} need you")
        ...
        if arow:
            ...
            bh_a = 3                     # just "the corral is empty"
        if y + bh_a <= h - 4:
            draw_box(win, y, 2, bh_a, cardw, "THE HERD")
            yy = y + 1
            if not arow:
                safe_addstr(win, yy, 4, "the corral is empty", cp(C_DIM))
            else:
...
def onboarding(win):
    ...
    while True:
        ...
        if w >= bw + 4:
            ...
            safe_addstr(win, y, max(2, (w - 6) // 2), "CORRAL",
        ...
        cmd = f'python3 "{SELF}"'
        center(cmd if len(cmd) < w - 4 else "python3 …/corral-tui.py",
...
HELP = [
    ...
    ("herd", "THE HERD — Claude agents on this Mac; click to focus"),
...
def _sig(d):
    ...
    ag_sig = (tuple(sorted((str(x.get("session_id")), str(x.get("status")),
              ...
              herd.get("working"), herd.get("needs_you"),
              herd.get("done_today"))
...
def main():
    if not sys.stdout.isatty():
        print("corral-tui needs an interactive terminal.", file=sys.stderr)
        sys.exit(1)
    ensure_daemon()
